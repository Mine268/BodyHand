...
namespace BodyHand {
    ...
    std::vector<float> HaMeROnnx::postProcessHandPosition(std::vector<Ort::Value>& output_tensor, int lr_flag) {
        ...
        Ort::Value& output_joint_positions_tensor = output_tensor[0];  // 获取输出张量 joint_positions    
        auto tensor_hand_info = output_joint_positions_tensor.GetTensorTypeAndShapeInfo();
        auto tensor_hand_shape = tensor_hand_info.GetShape();  // 获取张量的维度
        auto data_ptr = output_joint_positions_tensor.GetTensorData<float>();  // 获取张量数据指针
        if (data_ptr != nullptr) {
            size_t data_size = tensor_hand_info.GetElementCount();  // 获取元素数量
            for (int index = 0; index < data_size / 3; ++index) {
                if (lr_flag == 0) hand_position[63 * lr_flag + index * 3] = -1 * data_ptr[index * 3 + 1]; // 左手翻转 x :
                else if (lr_flag == 1) hand_position[63 * lr_flag + index * 3] = data_ptr[index * 3 + 1]; // 右手 x 
    ...
    HaMeROnnx::postProcessHandRotation(std::vector<Ort::Value>& output_tensor, int lr_flag) {
        ...
        Ort::Value& output_joint_rotations_tensor = output_tensor[1];  // 获取输出张量 joint_rotations
        auto tensor_rotations_info = output_joint_rotations_tensor.GetTensorTypeAndShapeInfo();
        auto tensor_rotations_shape = tensor_rotations_info.GetShape();  // 获取张量的维度
        auto rotations_ptr = output_joint_rotations_tensor.GetTensorData<float>();  // 获取张量数据指针
        if (rotations_ptr != nullptr) {
            size_t rotations_size = tensor_rotations_info.GetElementCount();  // 获取元素数量
        ...
        if (lr_flag == 0) { // 左手翻转
            for (int index = 0; index < 16; ++index) {
                // 提取 3×3 旋转矩阵 
                cv::Mat R = cv::Mat(3, 3, CV_32F, &hand_rotation[9 * index]).clone(); // 避免直接修改 handRotation
                // 轴角表示
                cv::Mat axis_angle;
            ...
            for (int index = 0; index < 16; ++index) {
                // 提取 3×3 旋转矩阵 
                cv::Mat R = cv::Mat(3, 3, CV_32F, &hand_rotation[144 + 9 * index]).clone(); // 避免直接修改 handRotation							
                // 轴角表示
                cv::Mat axis_angle;
                cv::Rodrigues(R, axis_angle);
    ...
    std::vector<float> HaMeROnnx::postProcessHandShape(std::vector<Ort::Value>& output_tensor, int lr_flag) {
        ...
        Ort::Value& output_hand_shape_tensor = output_tensor[2];  // 获取输出张量 hand_shape
        auto tensor_shape_info = output_hand_shape_tensor.GetTensorTypeAndShapeInfo();
        auto tensor_shape_shape = tensor_shape_info.GetShape();  // 获取张量的维度
        auto shape_ptr = output_hand_shape_tensor.GetTensorData<float>();  // 获取张量数据指针
        if (shape_ptr != nullptr) {
            size_t shape_size = tensor_shape_info.GetElementCount(); // 获取元素数量
    ...
    std::vector<float> HaMeROnnx::postProcessHand2D(std::vector<Ort::Value>& output_tensor, int lr_flag) {
        ...
        Ort::Value& output_joint2d_tensor = output_tensor[4];  // 获取输出张量 hand_shape
        auto tensor_joint2d_info = output_joint2d_tensor.GetTensorTypeAndShapeInfo();
        auto tensor_joint2d_shape = tensor_joint2d_info.GetShape();  // 获取张量的维度
        auto joint2d_ptr = output_joint2d_tensor.GetTensorData<float>();  // 获取张量数据指针
        if (joint2d_ptr != nullptr) {
            size_t joints2d_size = tensor_joint2d_info.GetElementCount();  // 获取元素数量
            for (int i = 0; i < 21; ++i) {
    ...
    std::vector<const char*> HaMeROnnx::getInputNodeNames(Ort::Session* session) {
        ...
        for (size_t i = 0; i < num_input_nodes; i++) {
            ...
            strcpy_s(name_copy, name_str.size() + 1, name_str.c_str());
    ...
    std::vector<Ort::Value> HaMeROnnx::detectHandFromBox(cv::Mat& hand_img) {
        ...
        std::vector<float> input_hand_data(1 * 3 * 256 * 256, 1.0f); // 填充数据
        // 将数据填充到 input_hand_data 中	
        for (int c = 0; c < 3; ++c) {
        ...
        Ort::Value input_hand_tensor = Ort::Value::CreateTensor<float>(
            _memory_info,
            input_hand_data.data(),       // 输入数据
            input_hand_data.size(),       // 数据的大小
            input_hand_shape.data(),      // 输入的形状
            input_hand_shape.size()       // 形状的维度数量
        );
        //std::vector<const char*> input_hand_node_names = { "input" }; // 输入节点名称
        ...
        //std::vector<const char*> output_hand_node_names = { "joint_positions", "joint_rotations", "hand_shape", "camera_trans", "joint_img" }; // 输出节点名称
    ...
    std::tuple<bool, bool> HaMeROnnx::detectPose(
        ...
        for (size_t box_ind = 0; box_ind < boxes_num; ++box_ind) {
            float classid = output_data[box_ind * 7 + 1]; // 类别ID
            float score = output_data[box_ind * 7 + 2]; // 置信度
